...
- [사진 업로드 화면]
    ...
    - [ ]  이미지 파일 선택시 선택한 이미지 파일들을 아래에 썸네일로 보여준다.
    - [ ]  수정 건일 경우 기존에 저장했던 이미지 파일들도 썸네일로 보여준다.
    - [ ]  썸네일 이미지 오른쪽 상단에 X버튼을 추가하여 클릭시 CASE1) 해당 이미지가 DB에 저장되어 있으면 비동기적으로 삭제 후 썸네일 이미지를 지운다. CASE2) 해당 이미지가 DB에 저장되어 있지 않은 경우 해당 썸네일 이미지만 지운다.
